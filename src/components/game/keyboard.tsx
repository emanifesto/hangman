...
export default function Keyboard({ keyPress }: {keyPress: Function}){
    return(
        <div className="bg-[#cad9fa] p-5 rounded-xl content-center hidden col-span-8 row-span-5 inset-shadow-sm inset-shadow-[#5078b4] lg:block">
...
function Key( {text, keyPress}:{text: string, keyPress: Function}){
    const keyAnimation: string = "ease-out active:translate-0.75 duration-300 "
    return(
        <div>
            <button className="relative z-0" onClick={() => keyPress(text)}>
                <div id={text} className={`${keyAnimation} size-12 lg:size-14 xl:size-18 content-center place-items-center bg-[#79abf2] rounded-md font-bold`}>
                    <p className="size-fit select-none">{text.startsWith('Key') ? text[3]: text}</p>
                </div>
                <div className="size-full  bg-[#5078b4] absolute top-1 left-1 rounded-md -z-1 border-2">
                </div>
            </button>
        </div>
    )
